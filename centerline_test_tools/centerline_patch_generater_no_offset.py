...
def creat_data(max_points, path_name, spacing_path, gap_size, save_num):
    ...
    image_pre_fix = path_name + \
        ('0' if i < 10 else '') + str(i) + '/' + 'image' + '0' + str(i)
            ...
            if j % gap_size == 0:
                ...
                if org_x_pixel != last_center_x_pixel or org_y_pixel != last_center_y_pixel or org_z_pixel != last_center_z_pixel:
                    ...
                    if pre_ind != -1 and next_ind != -1:
                        ...
                        folder_path = './patch_data/centerline_patch/no_offset/point_' + \
                            str(max_points) + '_gp_' + \
                            str(gap_size)+'/'+'d'+str(i)
                        if not os.path.exists(folder_path):
                            os.makedirs(folder_path)
                        record_name = 'centerline_patch/no_offset/point_' + str(max_points) + '_gp_' + str(gap_size)+'/'+'d'+str(
                            i)+'/' + 'd_' + str(i) + '_' + 'v_' + str(v) + '_' + 'patch_%d' % counter+'.nii.gz'
...
max_points = 500
gap_size = 1
path_name = 'test_data_part1/dataset'
