...
def interpolate_img(img, coords, order=3, mode='nearest', cval=0.0, is_seg=False):
    if is_seg and order != 0:
        ...
        for i, c in enumerate(unique_labels):
            res_new = map_coordinates((img == c).astype(
                float), coords, order=order, mode=mode, cval=cval)
...
def rotate_augmentation(data, pre_ind, next_ind, rotate_center, center, p_rot_per_axis=1, angle_x=(0, 2 * np.pi), angle_y=(0, 2 * np.pi), angle_z=(0, 2 * np.pi), border_mode_data='constant', border_cval_data=0.0, order_data=3):
    ...
    if modified_coords:
        for d in range(dim):
            ...
            coords[d] += ctr
        data_result = interpolate_img(
            data, coords, order_data, border_mode_data, cval=border_cval_data)

    vector_center = np.array(rotate_center)
    new_pre_cood = get_rotate_res(
        vector_center, center[pre_ind], a_x, a_y, a_z)
    new_next_cood = get_rotate_res(
        vector_center, center[next_ind], a_x, a_y, a_z)
