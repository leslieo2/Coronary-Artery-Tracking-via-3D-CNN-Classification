        ...
        for ih in range(int((h - cut_size) / move_step + 1)):
            for iw in range(int((w - cut_size) / move_step + 1)):
                ...
                if curr_proximity <= 0.0:
                    ...
                    folder_path = './patch_data/seeds_patch/negative/' + \
                        'gp_' + str(move_step)+'/d'+str(i)
                    if not os.path.exists(folder_path):
                        os.makedirs(folder_path)
                    record_name = 'seeds_patch/negative/' + 'gp_' + str(move_step)+'/d'+str(i)+'/' + 'd_' + str(
                        i) + '_' + 'x_' + str(center_x) + '_y_'+str(center_y)+'_z_'+str(center_z)+'.nii.gz'
                    # print(record_name)
...
def create_patch_images(path_name, spacing_path, cut_size=19, move_step=19):
    for i in range(8):
        patch_name, proximity_list = creat_data(
            path_name, spacing_path, i, cut_size, move_step)
