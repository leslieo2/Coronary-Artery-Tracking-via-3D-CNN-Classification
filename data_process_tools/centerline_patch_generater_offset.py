    ...
    for v in range(4):
        ...
        for j in range(start_ind, end_ind + 1):
            if j % gap_size == 0:
                ...
                org_x_pixel = get_spacing_res2(
                    center_x, spacing_x, resize_factor[1])
                org_y_pixel = get_spacing_res2(
                    center_y, spacing_y, resize_factor[2])
                org_z_pixel = get_spacing_res2(
                        ...
                        if pre_ind != -1 and next_ind != -1:
                            ...
                            if np.random.uniform() <= rotate_prob:
                                ...
                                new_src_arr, new_pre_cood, new_next_cood = rotate_augmentation(new_src_arr,
                                                                                               ...
                                                                                               center,
                                                                                               angle_x=(
                                                                                                   -60. / 360 * 2. * np.pi, 60. / 360 * 2. * np.pi),
                                                                                               angle_y=(
                                                                                                   -60. / 360 * 2. * np.pi, 60. / 360 * 2. * np.pi),
                                                                                               angle_z=(-60. / 360 * 2. * np.pi, 60. / 360 * 2. * np.pi))
                                p = [new_pre_cood[0],
                                     new_pre_cood[1], new_pre_cood[2]]
                                pre_sim = find_closer_point_angle(
                                    shell_arr, p, curr_c)
                                p = [new_next_cood[0],
                                     new_next_cood[1], new_next_cood[2]]
                                ...
                                next_ind_list.append(next_sim)
                                radials_list.append(new_radial)
                            folder_path = './patch_data/centerline_patch/offset/point_' + \
                                str(max_points) + '_gp_' + \
                                str(gap_size)+'/'+'d'+str(i)
                            if not os.path.exists(folder_path):
                                os.makedirs(folder_path)
                            record_name = 'centerline_patch/offset/point_' + str(max_points) + '_gp_' + str(gap_size)+'/'+'d'+str(
                                i)+'/' + 'd_' + str(i) + '_' + 'v_' + str(v) + '_' + 'patch_%d_' % counter+str(k)+'.nii.gz'
...
def create_patch_images(max_points, path_name, spacing_path, gap_size):
    for i in range(8):
        pre_ind_list, next_ind_list, radials_list, patch_name = creat_data(
            max_points, path_name, spacing_path, gap_size, i)
        ...
        dataframe.to_csv(csv_name, index=False, columns=[
                         'patch_name', 'pre_ind', 'next_ind', 'radials'], sep=',')
